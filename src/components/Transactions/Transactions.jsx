...
const TransactionPage = () => {
  ...
  useEffect(() => {
    const fetchTransactions = async () => {
      ...
      try {
        const response = await axios.get('https://Robotspace-backend-0ejj.onrender.com/api/transactions', {
  ...
  const handleTransactionSubmit = async (e) => {
    ...
    try {
      const response = await axios.post('https://Robotspace-backend-0ejj.onrender.com/api/transactions', newTransaction, {
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  const handleStatusUpdate = async (transactionId, newStatus) => {
    try {
      await axios.put(
        `https://Robotspace-backend-0ejj.onrender.com/api/transactions/${transactionId}`,
        { status: newStatus },
        {
          headers: { Authorization: `Bearer ${token}` },
        }
